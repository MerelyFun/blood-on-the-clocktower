...
export function AvatarFrame({children}:{children?:ReactNode}){return <span className="portrait-frame"><img src={artUrl('art/table/round-avatar.webp')} alt="" className={`portrait-border ${children?'':'player-avatar'}`} width={192} height={192} decoding="async"/>{children&&<span className="portrait-content">{children}</span>}</span>}
export function TownBoard({seats,phase,round,selected,onSelect,centerAction,winner,showAllMarks=false,compactNotes=false,onStoryteller}:{seats:DisplaySeat[];phase:string;round:number;selected?:string;onSelect?:(id:string)=>void;list?:boolean;centerAction?:ReactNode;winner?:string;showAllMarks?:boolean;compactNotes?:boolean;onStoryteller?:()=>void}){
 ...
 <span className="seat-art"><AvatarFrame>{s.roleData?<RoleToken role={s.roleData} alignment={s.alignment}/>:undefined}</AvatarFrame>{!s.alive&&!s.executed&&<img className="death-slash" src={artUrl('art/table/death-slash.webp')} alt="死亡"/>}{compactNotes&&s.executed&&<Axe className="execution-mark" aria-label="处决"/>}</span><strong><span className="seat-index">{s.index}</span>{s.name}</strong>{(!compactNotes||s.role)&&<span className="seat-role">{s.role||(s.left?'已离场':s.alive?'存活':'死亡')}</span>}{compactNotes?<span className="seat-flags note-seat-states">{s.alignment==='good'&&<small><Shield size={11}/>正义</small>}{s.alignment==='evil'&&<small><Flame size={11}/>邪恶</small>}{s.executed&&<small><Axe size={11}/>处决</small>}{!s.alive&&<small><Skull size={11}/>死亡</small>}{s.left&&<small>离场</small>}{s.traveller&&<small>旅行者</small>}{s.pendingDeath&&<small>待公布死亡</small>}{!s.voteAvailable&&<small>死者票已用</small>}</span>:<span className="seat-flags">{s.pendingDeath&&<StatusIcon status="pending-death" size={18} label="待公布死亡"/>}{s.traveller&&<small>旅行者</small>}{!s.alive&&<><Skull size={13}/><StatusIcon status={s.voteAvailable?"ghost-vote-available":"ghost-vote-used"} size={16} label={s.voteAvailable?"死者票可用":"死者票已用"}/></>}</span>}{!!s.marks?.length&&<span className="seat-markers"><>{showAllMarks?s.marks.map((mark,index)=><small key={`${mark}-${index}`}>{mark}</small>):<small>{s.marks[0]}{s.marks.length>1?` +${s.marks.length-1}`:''}</small>}</></span>}</button>})}</div></div>
}
