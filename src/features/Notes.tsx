...
import { ArrowLeft, ArrowRight, BookOpen, ChevronLeft, ChevronRight, MoreHorizontal, NotebookPen, Plus, Search, Star, Trash2, X } from 'lucide-react';
import { useApp } from '../lib/context';
import { CATALOG, publicGame, uid, type PublicGame, type Role, type RoomView } from '../lib/domain';
...
import { createNotebook, importNotebook, markAt, playerAnnotations, stateAt, syncPublicRoom, type Notebook, type NoteEntry, type NotePlayer, type RoleMark } from '../lib/notebooks';
...
export function Notes({id}:{id?:string}) {
  ...
  return <main className="page notes-library">
    <div className="section-heading"><div><h1>我的笔记</h1></div><NotebookPen size={28}/></div>
...
function NotebookEditor({initial,context,room:providedRoom,mySeatId,embedded=false}:{initial:NotebookDraft;context:NotebookContext;room?:PublicGame;mySeatId?:string;embedded?:boolean}) {
  ...
  const seats:DisplaySeat[]=visible.map(p=>{
    const annotation=playerAnnotations(book,p.id,day),state=stateAt(book,p.id,day);
    return {id:p.id,index:p.index,name:`${p.name}${book.myPlayerId===p.id?' · 我':''}`,alive:state?.alive??true,voteAvailable:state?.voteAvailable??true,left:state?.left??false,traveller:p.traveller,role:annotation.primary?roleName(book,annotation.primary):'未标记',roleData:[...bRoles(book)].find(r=>r.id===annotation.primary),marks:annotation.extras.map(v=>roleName(book,v))};
  });
  ...
  const person=visible.find(p=>p.id===selected)||book.players.find(p=>p.id===selected);
  ...
  return <main className={`notebook-page ${embedded?'embedded':''}`}>
    ...
    <div className="notebook-workspace"><div className="notebook-table-column"><div className="notebook-table"><div className="note-table-controls"><button aria-label="逆时针旋转座位" onClick={()=>rotate(1)}>↶ 旋转</button><button aria-label="顺时针旋转座位" onClick={()=>rotate(-1)}>旋转 ↷</button></div><TownBoard showAllMarks seats={arrangedSeats} phase="day" round={day} centerAction={<Button className="note-next-day" disabled={!!book.room||book.currentDay>=365} title={book.room?'关联笔记的日期随房间同步':undefined} onClick={()=>update(b=>({...b,currentDay:b.currentDay+1,phase:'day'}))}>{book.room?'随房间进入下一天':'进入下一天'}</Button>} selected={selected} onSelect={id=>{setSelected(id);setEditing(undefined);}}/></div>
      <section className="note-general">
        <div className="note-section-heading"><h2>笔记</h2><Button variant="ghost" onClick={()=>addEntry('free')}><Plus size={16}/>记一条</Button></div>
        <GroupedEntries book={book} entries={book.entries.filter(e=>e.kind!=='player')} update={update} editing={editing} setEditing={setEditing} onPlayer={setSelected}/>
      </section>
    </div>
    {!person&&<aside className="note-desktop-hint"><NotebookPen size={30}/><h2>点一位玩家</h2><p>标记角色与状态，查看和补充信息。</p></aside>}
    {person&&<PlayerPanel key={person.id} player={person} book={book} day={day} update={update} editing={editing} setEditing={setEditing} onPlayer={setSelected} onClose={()=>{setSelected(undefined);setEditing(undefined);}} onAdd={()=>addEntry('player',person.id)}/>}
    </div>
...
function visiblePlayers(book:Notebook,day:number){return book.players.filter(p=>(p.joinedDay??1)<=day&&!(stateAt(book,p.id,day)?.left??p.left??false)).map(p=>{const state=stateAt(book,p.id,day);return {...p,index:state?.index??p.index,name:p.noteName??state?.name??p.name};}).sort((a,b)=>a.index-b.index);}
...
function PlayerPanel({player,book,day,update,editing,setEditing,onPlayer,onClose,onAdd}:{player:NotePlayer;book:Notebook;day:number;update:Change;editing?:string;setEditing:(id?:string)=>void;onPlayer:(id:string)=>void;onClose:()=>void;onAdd:()=>void}) {
  ...
  function move(offset:number){const visible=visiblePlayers(book,day),i=visible.findIndex(p=>p.id===player.id);if(visible.length)onPlayer(visible[(i+offset+visible.length)%visible.length].id);setEditing(undefined);}
...
function RolePicker({book,playerId,day,kind,editId,update,onClose}:{book:Notebook;playerId:string;day:number;kind:RoleMark['kind'];editId?:string;update:Change;onClose:()=>void}) {
  const old=editId?book.marks.find(m=>m.id===editId):markAt(book,playerId,kind,day);
  const annotation=playerAnnotations(book,playerId,day);
  const multi=kind!=='primary';
  const [chosen,setChosen]=useState<string[]>((kind==='extra'&&!editId?undefined:old?.roles)||(kind==='primary'?(annotation.primary?[annotation.primary]:[]):kind==='extra'?[...annotation.extras,...(stateAt(book,playerId,day)?.alive===false?['死亡']:[]),...(stateAt(book,playerId,day)?.voteAvailable===false?['死者票已用']:[])]:[])),[query,setQuery]=useState(''),[all,setAll]=useState(!book.script),[ability,setAbility]=useState<Role>();
  const roles=useMemo(()=>{const map=new Map((all?[...CATALOG,...book.script?.roles||[]]:book.script?.roles||CATALOG).map(r=>[r.id,r]));return [...map.values()].filter(r=>r.name.includes(query)||r.id.includes(query.toLowerCase()));},[book.script,all,query]);
  function commit(values:string[]){update(b=>{const roles=kind==='extra'?values.filter(v=>!['死亡','死者票已用'].includes(v)):values;const marks=editId?b.marks.map(m=>m.id===editId?{...m,roles}:m):[...b.marks,{id:uid(),playerId,day,phase:b.phase,kind,roles,createdAt:now()}];const state=stateAt(b,playerId,day);return {...b,marks,states:kind==='extra'&&!book.room?[...b.states,{id:uid(),playerId,day,phase:b.phase,alive:!values.includes('死亡'),voteAvailable:!values.includes('死者票已用'),left:state?.left??false,index:state?.index,name:state?.name,createdAt:now()}]:b.states};});onClose();}
  function choose(value:string){if(book.room&&['死亡','死者票已用'].includes(value))return;if(!multi){commit([value]);return;}setChosen(v=>v.includes(value)?v.filter(x=>x!==value):[...v,value]);}
  return <Modal title={`${editId?'修正笔误：':''}${markKindLabel(kind)}`} onClose={onClose} footer={<div className="row between"><Button variant="ghost" onClick={()=>commit([])}>清空标记</Button>{multi&&<Button variant="primary" onClick={()=>commit(chosen)}>完成 · {chosen.length} 项</Button>}</div>}>
    <div className="note-role-controls"><label className="note-search"><Search size={17}/><input aria-label="搜索角色" value={query} maxLength={100} placeholder="搜索角色名称" onChange={e=>setQuery(e.target.value)}/></label>{book.script&&<button className="note-text-button" onClick={()=>setAll(!all)}>{all?'只看当前剧本':'搜索全部角色'}</button>}</div>
    {kind==='extra'&&<div className="note-quick">{['死亡','死者票已用','醉酒','中毒','善良','邪恶'].filter(v=>v.includes(query)).map(v=><button key={v} disabled={!!book.room&&['死亡','死者票已用'].includes(v)} aria-pressed={chosen.includes(v)} className={chosen.includes(v)?'active':''} onClick={()=>choose(v)}>{chosen.includes(v)?'✓ ':''}{v}</button>)}</div>}
    {multi&&chosen.length>0&&<div className="note-quick">{chosen.map(v=><button key={v} onClick={()=>choose(v)}>{roleName(book,v)} ×</button>)}</div>}
    <div className="note-role-picker">{roles.map(r=><div key={r.id}><button className={chosen.includes(r.id)?'selected':''} onClick={()=>choose(r.id)}><RoleToken role={r} small/><span>{r.name}</span></button><button aria-label={`查看${r.name}能力`} className="note-role-help" onClick={()=>setAbility(ability?.id===r.id?undefined:r)}>?</button>{ability?.id===r.id&&<p>{r.ability}</p>}</div>)}</div>
    {query.trim()&&!roles.some(r=>r.name===query.trim())&&<Button onClick={()=>choose(query.trim())}>使用自定义标记「{query.trim()}」</Button>}
  </Modal>;
}
