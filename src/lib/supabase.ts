import { localApi, localBackendEnabled, LocalApiError } from './localBackend';
import { createClient, type SupabaseClient } from '@supabase/supabase-js';
export interface CloudConfig {url:string;key:string;turnstile?:string;}
export function config():CloudConfig {let saved:Partial<CloudConfig>={};try{const value=JSON.parse(localStorage.getItem('bt-cloud')||'{}');if(value&&typeof value==='object'&&!Array.isArray(value))saved=value;}catch{}return {url:(typeof saved.url==='string'?saved.url:'')||import.meta.env.VITE_SUPABASE_URL||'',key:(typeof saved.key==='string'?saved.key:'')||import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY||'',turnstile:localBackendEnabled?'':(typeof saved.turnstile==='string'?saved.turnstile:'')||import.meta.env.VITE_TURNSTILE_SITE_KEY||''};}
...
export function supabase(){if(localBackendEnabled)return null;if(client)return client;const c=config();if(!c.url||!c.key)return null;try{validateConfig(c);client=createClient(c.url,c.key,{auth:{flowType:'pkce',detectSessionInUrl:true,persistSession:true,autoRefreshToken:true}});return client;}catch{return null;}}
...
export async function api<T>(body:Record<string,unknown>):Promise<T> {
 if(localBackendEnabled){try{const data=await localApi<T&{serverTime?:number}>(body);if(typeof data.serverTime==='number')clockOffset=data.serverTime-Date.now();return data;}catch(error){if(error instanceof LocalApiError)throw new ApiError(error.message,error.code);throw error;}}
