...
5. 未匹配的新角色可直接创建，填写名称、阵营和能力；重新匹配不会清除已选的新角色。
6. 剧本名称可留空，默认“未命名剧本”。核对后下载通用 JSON，或直接放入剧本工坊继续编辑。

识别结果必须人工核对。小字、花体、倾斜拍摄、低对比背景和图标遮挡都可能造成漏识别或误识别。工具先匹配已有角色目录，并保留未匹配文字供创建新角色。新角色可在图片导入窗口中直接填写或编辑，能力需人工核对，不会自动编造。
